...
var isAttribute = function(el, attr) {
    ...
    if (el.nodeName != "IMG"){
        if (attr =="width" || attr == "height") return false;
    }
...
var RE_GT = />/g;
var RE_AMP = /&/g;
var xmlEscape = function(t){
    //do this first
    text = t.toString().replace(RE_AMP, "&amp;");
...
var convertParsedToDom = function(document, dom){
    var el;
    if (dom.type == "text"){
        el = document.createTextNode(dom.data);
    } else {
        el = document.createElement(dom.name);
        addDomChildren(el, dom.children);
    }
    return el;
}

addDomChildren = function(el, domChildren){
    if (!domChildren) return;
    each(domChildren, function(child){
        el.appendChild(convertParsedToDom(document, child));
    });
}
...
domelement.__defineSetter__("innerHTML", function(html){
    ...
    addDomChildren(this, handler.dom);
